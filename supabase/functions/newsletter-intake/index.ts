 ...
 *
 * AND ONE MODE THAT IS A FEATURE RATHER THAN A REPAIR:
 *   {"group_status": true}
 *                       reads the published announcements, finds the most
 *                       recent one about home groups, shortens it to fit the
 *                       box on Connect where the group finder would be, and
 *                       writes it there. No mailbox, no queue: an admin taps a
 *                       button, and migration 0048's
 *                       hc_admin_refresh_group_status() is what they reach.
 *                       Every link, date, time and phone number in the
 *                       announcement has to survive the shortening or nothing
 *                       is written at all — see whatWasLost(), which is the
 *                       point of this mode having been written carefully.
 *
 * DEPLOY
...
/* ========================================================================
   group status
   The paragraph on Connect where the group finder would be, kept current
   from whatever the church last said about home groups.

   WHY THIS IS HERE AND NOT ITS OWN FUNCTION. It is the same job as the
   backfill: read announcements that already exist, ask the model one
   question about them, write the answer to a column. It needs this file's
   model configuration, its transient error handling and its secret, and a
   second deployment would be a second place for all three to drift. The
   mailbox is not opened on this path at all.

   WHAT IT MUST NOT DO, and this is the whole of the difference between this
   and every other mode. The paragraph it writes is on a public screen the
   moment it is written; there is no review queue in front of it. So the
   check below is not a nicety: a shortening that has lost the number you
   text to get into a group is worse than the between-seasons sentence it
   replaced, because it looks current and is useless. Anything the model
   drops, this refuses to write, and says which thing was dropped.
   ===================================================================== */
...
/* How many announcements to look at, and how long the answer may be.
   ...
   older one it likes the wording of better.

   300 CHARACTERS, because that box holds a paragraph. The seed sentence in
   js/data.js is 148 and reads as one comfortable block on a phone; twice that
   is the ceiling where it starts to be a wall. This is a cap and not a target,
...
/* Every link on an announcement, wherever it is kept.
   ...
   choice made here: link_url is the card's own button (0033), the anchors in
   body_html are the ones inside the words the intake wrote, and a bare URL
   typed into the plain body is what an admin writing by hand produces. A
...
/* ------------------------------------------------------------------------
   ...
   Three kinds of thing, and each one is checked the way it is actually
   written rather than by string equality, because "September 6" and "Sept 6"
   are the same date and a check that said otherwise would refuse every good
   answer the model gives.

   A LINK is checked exactly. There is one correct spelling of a URL and the
   model was handed it.

   A PHONE NUMBER is checked on its digits. (833) 801-3857 and 833-801-3857
   are the same number and the church writes it both ways.

   A DATE is checked as a month and a day, in any of the ways a person writes
   them: "September 6", "Sept 6", "Sep 6th", "9/6". A TIME the same way, with
   or without the minutes and the space. What this deliberately does not do is
   invent a year or resolve "this Sunday" — the model is told to keep those
   words as they are, and a relative date that survived word for word passes
   here because the words are what was checked.
   --------------------------------------------------------------------- */
...
/* Everything in the announcement that the shortened note has to still carry,
   as a list of complaints when it does not. An empty list is a pass. */
...
function groupStatusPrompt(rows: GroupRow[], today: string, retry: string[]): string {
  return [
    ...
    `screen headed "Home groups". At most ${GROUP_NOTE_MAX} characters, and shorter is`,
    ...
    `takes you past ${GROUP_NOTE_MAX} characters, drop adjectives and whole sentences of`,
...
async function askForGroupNote(
  apiKey: string,
  model: string,
  rows: GroupRow[],
  today: string,
  ...
  try {
    res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contents: [{ role: 'user', parts: [{ text: groupStatusPrompt(rows, today, retry) }] }],
          generationConfig: {
            temperature: 0.2,
            maxOutputTokens: 4096,
            responseMimeType: 'application/json',
            responseSchema: GROUP_STATUS_SCHEMA,
          },
        }),
      },
    );
  ...
  if (!res.ok) {
    if (res.status === 429) {
      throw new TransientError('Gemini is rate limiting us on the free tier. Nothing was changed, try again in a minute.');
    }
    if (res.status >= 500) {
      throw new TransientError(`Gemini is busy (${res.status}). Nothing was changed, try again in a minute.`);
    }
    const detail = await res.text().catch(() => '');
    throw new Error(`Gemini returned ${res.status}: ${detail.slice(0, 300)}`);
  }
...
async function runGroupStatus(
  ...
  const { data, error } = await admin
    .from('announcements')
    .select('id, title, body, body_html, link_url, image_url, published, created_at')
    .eq('published', true)
    .order('created_at', { ascending: false })
  ...
  /* One retry, and only for a dropped detail. The first answer is usually
     ...
     that works, and it costs one more call on a button somebody taps a few
     times a season. A second failure is reported rather than retried again:
     at that point the model is not going to keep the link, and the honest
     outcome is the box keeping the words it already had. */
  let answer = await askForGroupNote(apiKey, model, rows, today, []);
  ...
  if (answer.found !== true) {
    return {
      ok: true,
      changed: false,
      note: `Nothing in the last ${rows.length} announcements is about home groups, so the box was left as it is.`,
    };
  }
  ...
  /* Said it found one and then did not name it, or named one we did not send.
     ...
     above: "nothing was posted about home groups" would be a lie, and the
     person reading it would go looking for an announcement that is in fact
     sitting there. */
  if (!picked || !note) {
    return {
      ok: false,
  ...
  if (lost.length) {
    answer = await askForGroupNote(apiKey, model, rows, today, lost);
  ...
  if (lost.length) {
    // Not written. Named, so the admin knows what to type in by hand and does
    // not have to guess what the robot disliked about the announcement.
    return {
      ok: false,
      changed: false,
  ...
  /* The ceiling is checked here as well as asked for. A model that ran long
     is not a reason to refuse a good paragraph, but a paragraph that would
  ...
  if (note.length > GROUP_NOTE_MAX) {
    const cut = note.slice(0, GROUP_NOTE_MAX);
  ...
  if (String(previous ?? '').trim() === note && !seasonMoved) {
    return {
      ...
      note: 'The box already says this, so nothing was changed.',
    };
  }

  /* The flyer, when the announcement has one and the box does not already
     show that same picture. Carried over rather than uploaded: it is the
...
Deno.serve(async (req: Request) => {
  ...
  let body: {
    probe?: boolean; dry_run?: boolean; backfill?: boolean;
    group_status?: boolean; limit?: number;
  } = {};
  try { body = await req.json(); } catch { /* an empty body is a normal tick */ }
  const probe = body.probe === true;
  const dryRun = body.dry_run === true;
  const backfill = body.backfill === true;
  const groupStatus = body.group_status === true;
  ...
  /* The home groups box exits here, before the mailbox secrets are even
     checked, because it never opens a mailbox: it reads announcements that are
     already in the table, shortens the most recent one about home groups, and
     writes it to the paragraph on Connect. Migration 0048 and
     hc_admin_refresh_group_status() are the other half.

     IT ALWAYS WRITES A ROW, unlike the backfill above, and for the reason
     every other run row in this file exists: pg_net threw the response away
     the moment the request was accepted, and the Admin screen is watching
     group_status_runs for a row newer than the one it started with. A path out
     of here that wrote nothing would leave a button spinning for a minute and
     then apologising for a run that in fact succeeded. */
  if (groupStatus) {
    const settle = async (row: Record<string, unknown>) => {
      ...
      if (error) console.error('newsletter-intake group status: could not log the run:', error.message);
      return json(row);
    };
    ...
    try {
      return await settle(await runGroupStatus(admin, geminiKey, model));
    } catch (err) {
      console.error('newsletter-intake group status failed:', err);
      /* A busy model and a broken one read the same to a person standing at a
         ...
         difference is in the sentence TransientError carries, which names
         waiting a minute as the fix. */
      return await settle({
        ok: false, changed: false,
