...
| `reading_plans.starts_on`, `series_id` | the week number is arithmetic from them, and a plan that names its series has both written for it by the sermons it walks beside (`0055`). A typed date is a week number that disagrees with the sermon next to it, which is the failure this whole corner of the schema is about. The week's *reading* is a sentence and stays editable in place |
...
| `church_profile.groups_note_link_url`, `groups_note_link_label` | the button across the bottom of the home groups card: the link people tap to join. A destination, not a sentence, so it goes through `hc_admin_set_group_note` like the flyer beside it and is refused unless it is an http or https address. It is its own column because the one this church posts is 355 characters of query string, which no 300 character paragraph could ever have carried — the two runs that tried are in `group_status_runs`. Migration `0054` |
